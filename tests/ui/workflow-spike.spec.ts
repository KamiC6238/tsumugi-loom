...
test('keeps workflow selection in sync between the canvas and side panel', async ({ page }) => {
  ...
  await expect(page.getByTestId('stage-node-docs-reconciler')).toBeVisible()

  await expect(page.getByTestId('selected-stage-title')).toHaveText('Coding')
  ...
  await expect(page.getByTestId('selected-stage-title')).toHaveText('Review')
  ...
  await expect(page.getByTestId('selected-stage-title')).toHaveText('Test')
