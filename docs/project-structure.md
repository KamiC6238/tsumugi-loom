...
3. `src/stores/`：Pinia store 层，当前承载 workflow 会话状态、全局 added skills 状态、GitHub tasks 状态、workflow run 状态与 Knowledge Base 更新状态。
4. `src/lib/`：与视图解耦的纯状态与辅助逻辑，包括 workflow 状态变换、skill catalog 解析、GitHub tasks helper、workflow run payload 和 Knowledge Base readiness。
...
12. `artifacts/runs/`：本地 runner 生成的 issue workflow Run 产物，包含 input snapshot、skill snapshot、节点 prompt、日志、node-result 和 Knowledge Base update artifact。
...
| Knowledge Base 更新 | `src/lib/knowledgeBase.ts`, `src/stores/knowledgeBase.ts`, `src/stores/workflowRuns.ts`, `TasksPanel.vue`, `scripts/loom/copilot-runner.mjs` | [knowledge-base-updates.md](knowledge-base-updates.md) |
...
5. `docs/knowledge-base-updates.md` 记录 issue Run 后 Knowledge Base 更新的 readiness、runner endpoint、文档写入和状态契约。
6. `docs/color-mode.md` 记录主题偏好、localStorage key 和 document root 同步规则。
7. `docs/project-structure.md` 记录目录职责、文档归属和 workflow artifact 约定。
8. `docs/validation-map.md` 记录测试入口、覆盖范围和模块验证归属。
9. `artifacts/workflows/` 记录单次 workflow 的上下文、验证结果和回合内说明；长期稳定事实归入对应长期文档。
...
10. Knowledge Base 更新：前端先用 `POST /repository` 推送 selected repository，再通过本地 runner `POST /runs/:runId/knowledge-base` 写入 runner current repository 的 `docs/knowledge-base.md`。
...
5. `artifacts/runs/<run-id>/` 由本地 Copilot runner 生成，保存 issue workflow Run 的 input、skill snapshot、节点 prompt/log/result、run manifest 和 `knowledge-base/update.json`；这些是执行记录，不替代 `artifacts/workflows/` 的开发回合产物。
6. 目标仓库的 `docs/knowledge-base.md` 是 Knowledge Base 更新的默认写入位置；runner 会保留用户手写内容，并用 Tsumugi Loom marker 管理自动生成条目。
