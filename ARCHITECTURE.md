...
1. src/main.ts
   应用入口，加载全局样式，安装 Pinia，并挂载 Vue 应用。
2. src/style.css
...
4. src/composables/useWorkflowStudio.ts
   workflow 页面级编排层，集中管理 workflow 集合、活动 workflow、当前右侧面板、创建对话框开关、当前选中节点，并通过 Pinia skills store 暴露已添加 skills，以及 create/select/save node/open skills/close drawer 等领域动作。
...
8. src/stores/skills.ts
   Pinia skills store，持有用户已添加的 skill ids，并派生 added skills、added node skills 与添加状态判断。
9. src/components/ui/
...
2. src/composables/
   页面级状态编排与组合逻辑。
3. src/stores/
   Pinia store 层，当前承载全局 added skills 状态。
4. src/lib/
...
2. src/main.ts 安装 Pinia、挂载 App.vue，同时引入 src/style.css 提供全局样式层。
...
8. useWorkflowStudio.createWorkflow 调用 appendWorkflow 生成新的 WorkflowRecord，并在成功创建后切回 workflow 面板、关闭对话框和节点 Drawer；activateWorkflow 调用 selectWorkflow 切换 activeWorkflowId，并同步清理旧 workflow 的节点编辑态。
...
11. SkillsPanel 接收全局 skillCatalog 与 Pinia store 中的 addedSkillIds，按 macro/node 分组渲染卡片；卡片 Switch 只发出 toggleSkill 意图，实际添加状态由 Pinia skills store 更新。
12. useWorkflowStudio.openNodeDrawer 先确认节点属于当前活动 workflow，再只记录 selectedNodeId；WorkflowNodeDrawer 接收派生后的 selectedNode、open 状态与 addedNodeSkills，保存时 trim 输入并通过 saveSelectedNode 回写节点标签与可选 node skill，关闭时清空节点编辑态。
...
4. tests/logic/workflow-studio.test.ts 校验页面级 composable 的全局面板、Pinia skill 添加状态和节点 skill 保存规则。
5. tests/logic/workflow-ui.test.ts 用 Vue Test Utils 校验 SkillsPanel、Sidebar、App 与 Drawer 的组件级接线。
6. tests/ui/workflow-sidebar.spec.ts 校验创建 workflow、切换活动画布、节点 Drawer 改名、added node skill select，以及 skill 卡片文本 containment 等关键交互。
