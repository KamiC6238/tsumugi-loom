...
6. 用 Vitest 和 Playwright 覆盖状态逻辑与关键 UI 交互。
...
3. 侧边栏、创建对话框、节点 Drawer 和主画布组成的 UI shell。
...
4. src/composables/useWorkflowStudio.ts
   workflow 页面级编排层，集中管理 workflow 集合、活动 workflow、创建对话框开关、当前选中节点，以及 create/select/rename/close drawer 等领域动作。
5. src/components/workflow-studio/
   workflow feature 组件层，其中 WorkflowSidebar 负责列表与入口按钮，WorkflowCanvasPanel 负责活动 workflow 的明细、画布与节点点击事件，CreateWorkflowDialog 负责创建表单与草稿输入，WorkflowNodeDrawer 负责节点编辑抽屉。
6. src/lib/workflows.ts
   纯函数状态层，负责创建空状态、追加 workflow、切换活动 workflow、节点改名以及解析当前活动 workflow。
7. src/components/ui/
   复用型 UI 原语封装，目前主要承载按钮、输入框、标签、对话框和 Drawer。
...
7. useWorkflowStudio.createWorkflow 调用 appendWorkflow 生成新的 WorkflowRecord，并在成功创建后关闭对话框和节点 Drawer；activateWorkflow 调用 selectWorkflow 切换 activeWorkflowId，并同步清理旧 workflow 的节点编辑态。
8. WorkflowCanvasPanel 接收 activeWorkflow，并把 nodes 与 edges 传给 Vue Flow 渲染；点击节点时向上 emit nodeClick，当没有活动 workflow 时显示空状态。
9. useWorkflowStudio.openNodeDrawer 先确认节点属于当前活动 workflow，再只记录 selectedNodeId；WorkflowNodeDrawer 接收派生后的 selectedNode 与 open 状态，保存时 trim 输入并通过 renameSelectedNode 回写节点标签，关闭时清空节点编辑态。

当前仓库级验证流如下：

1. tests/logic/workflow-state.test.ts 校验 workflow 状态变换的纯函数契约。
2. tests/ui/workflow-sidebar.spec.ts 校验创建 workflow、切换活动画布、节点 Drawer 改名，以及切换 workflow 或关闭 Drawer 时清理节点编辑态等关键交互。
